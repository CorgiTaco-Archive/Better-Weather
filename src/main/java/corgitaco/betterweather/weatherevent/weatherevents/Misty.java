...
public class Misty extends WeatherEvent {

    private final float[] rainSizeX = new float[1024];
    private final float[] rainSizeZ = new float[1024];

    public Misty() {
        super(WeatherEventSystem.MISTY, 0.5);
        for (int i = 0; i < 32; ++i) {
            for (int j = 0; j < 32; ++j) {
                float f = (float) (j - 16);
                float f1 = (float) (i - 16);
    ...
    public boolean renderWeather(Minecraft mc, ClientWorld world, LightTexture lightTexture, int ticks, float partialTicks, double x, double y, double z) {
        float rainStrength = (world.getRainStrength(partialTicks) * 0.5F);
        ...
        for (int graphicQualityZ = floorZ - graphicsQuality; graphicQualityZ <= floorZ + graphicsQuality; ++graphicQualityZ) {
            for (int graphicQualityX = floorX - graphicsQuality; graphicQualityX <= floorX + graphicsQuality; ++graphicQualityX) {
                ...
                double rainSizeX = (double) this.rainSizeX[rainSizeIdx] * 0.1D;
                double rainSizeZ = (double) this.rainSizeZ[rainSizeIdx] * 0.1D;
                ...
                if (floorYMinusGraphicsQuality != floorYPlusGraphicsQuality) {
                    ...
                    if (i1 != 1) {
                        ...
                        i1 = 1;
                        mc.getTextureManager().bindTexture(WorldRenderer.RAIN_TEXTURES);
                        bufferbuilder.begin(7, DefaultVertexFormats.PARTICLE_POSITION_TEX_COLOR_LMAP);
                    }

                    float f7 = (float) (random.nextDouble() + (double) (ticksAndPartialTicks * (float) random.nextGaussian()) * 0.03D);
                    float fallSpeed = (float) (random.nextDouble() + (double) (ticksAndPartialTicks * (float) random.nextGaussian()) * 0.05D);
                    ...
                    int j4 = (l3 * 3 + 240) / 4;
                    int k4 = (i4 * 3 + 240) / 4;
                    if (Misty.DoesItMistInDeserts(biome)) {
    ...
    public static boolean DoesItMistInDeserts(Biome biome) {
        if (!BetterWeatherConfig.doesMistOccurInDeserts.get())
